...
if st.session_state.section_index == 0:
    with st.container():
        ...
        with aboutme_section:
            st.markdown(f"""<div style="color: rgb(251,190,91); font-weight: bold;">""" "About" """</div>""",  unsafe_allow_html=True)
            st.markdown(f"""<div style="color: rgb(251,190,91);">{about}</div>""", unsafe_allow_html=True)
        
            

elif st.session_state.section_index == 1:
   with st.container(f"""<div style="color: rgb(251,190,91); font-weight: bold;"></div>""",  unsafe_allow_html=True):
